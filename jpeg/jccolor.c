           ...
           JDIMENSION output_row, int num_rows)
{
  ...
  while (--num_rows >= 0) {
    ...
    output_row++;
    for (col = 0; col < num_cols; col++) {
      uint16_t pixel = *((uint16_t*)(inptr));
	  outptr0[col] = ((pixel >> 8) & 0xF8);
	  outptr1[col] = ((pixel >> 3) & 0xFC);
	  outptr2[col] = ((pixel & 0x1F) << 3);
...
jinit_color_converter (j_compress_ptr cinfo)
{
  ...
  case JCS_RGB:
    if(cinfo->in_color_space == JCS_RGB_565)
		cconvert->pub.color_convert = rgb_565_rgb_convert;
    else if (cinfo->num_components != 3)
      ERREXIT(cinfo, JERR_BAD_J_COLORSPACE);
    else if (cinfo->in_color_space == JCS_RGB && RGB_PIXELSIZE == 3)
